...
def get_all_run_durations(ratios_year):
    ...
    for label in ratios_year:
        for lbc in ratios_year[label]:
            run_data = ratios_year[label][lbc]
            for run in run_data:
...
def get_ratio_mean_std(ratios):
    ...
    for label in ratios:
        for lbc in ratios[label]:
            run_ratios = ratios[label][lbc]
            for run in run_ratios:
                for det in run_ratios[run]:
                    for bc_type in run_ratios[run][det]:
                        ratio_means[label][lbc][run][det][bc_type] = ratios[label][lbc][run][det][bc_type].mean()
                        ratio_stds[label][lbc][run][det][bc_type] = ratios[label][lbc][run][det][bc_type].std()

    return ratio_means, ratio_stds

def draw_ratio_vs_lbc(ratios, output_folder, bc_type_sel, master_durations):
    ...
    for label in ratios_mean:
        ...
        for lbc in sorted(ratios_mean[label].keys()):
            ratio_data = ratios_mean[label][lbc]
            ...
            for run in sorted(ratio_data.keys(), key=int):
                ...
                for det in run_ratios:
                    ...
                    if bc_type_sel in run_ratios[det]:
                        ...
                        stds[key][bc_type_sel].append(ratios_std[label][lbc][run][det][bc_type_sel])
            ...
            for det_key in means:
                if bc_type_sel in means[det_key] and len(means[det_key][bc_type_sel]) > 0:
                    plt.errorbar(
                        ...
                        fmt='o', label=f'{label}, {det_key}, LBC={lbc}, bc_type={bc_type_sel}'
                    )
...
def draw_ratio_vs_bc_type(ratios, output_folder, lbc_sel, master_durations):
    ratios_mean, ratios_std = get_ratio_mean_std(ratios)
    ...
    for label in ratios_mean:
        ...
        ratio_data = ratios_mean[label][lbc_sel]
        # Identify reference detector (assumes consistent naming)
        first_run = list(ratio_data.keys())[0]
        ref_detector = [d for d in ratio_data[first_run] if len(ratio_data[first_run][d].keys()) == 0][0]
        ...
        for run in sorted(ratio_data.keys(), key=int):
            ...
            for det in run_ratios:
                ...
                for bc_type in run_ratios[det]:
                    ...
                    stds[key][bc_type].append(ratios_std[label][lbc_sel][run][det][bc_type])
        ...
        for det_key in means:
            for bc_type in means[det_key]:
                plt.errorbar(
                    ...
                    xerr=x_errors_master[plot_indices],
                    fmt='o', label=f'{label}, {det_key}, bc_type={bc_type}, lbc={lbc_sel}'
                )
                if label == "vdm":
        ...
        plt.legend(fontsize=(sum(durations_list)*0.00015 + 10), loc='upper right')
        plt.tight_layout()
        plt.savefig(output_folder / f"year_summary_{label}_lbc_{lbc_sel}.pdf")
        plt.savefig(output_folder / f"year_summary_{label}_lbc_{lbc_sel}.png", dpi=600)
        plt.close()

def draw_ratio_vs_label(ratios, output_folder, bc_type_sel, lbc_sel, master_durations):
    ratios_mean, ratios_std = get_ratio_mean_std(ratios)
    ...
    for label in ratios_mean:
        ...
        for run in sorted(ratio_data.keys(), key=int):
            ...
            for det in run_ratios:
                ...
                means[key][bc_type_sel].append(run_ratios[det][bc_type_sel])
                stds[key][bc_type_sel].append(ratios_std[label][lbc_sel][run][det][bc_type_sel])
        ...
        for det_key in means:
            plt.errorbar(
                ...
                yerr=stds[det_key][bc_type_sel],
                xerr=x_errors_master[plot_indices],
                    fmt='o', label=f'{label}, {det_key}, lbc={lbc_sel}, bc_type={bc_type_sel}'
                )
    ...
    plt.legend(fontsize=(sum(durations_list)*0.00015 + 10), loc='upper right')
    plt.tight_layout()
    plt.savefig(output_folder / f"year_summary_lbc_{lbc_sel}_{bc_type_sel}.pdf")
    plt.savefig(output_folder / f"year_summary_lbc_{lbc_sel}_{bc_type_sel}.png", dpi=600)
    plt.close()
...
def draw_year_summary(year, config_analysis, config_downloader):
    ...
    for label in labels:
        ...
        for lbc in lbcs:
            ratios[label][lbc] = {}
            lbcs_all.add(lbc)
            with open(label_folder / f"lbc_{lbc}" / "runs" / "ratios.pkl", "rb") as f:
                ratios[label][lbc] = pickle.load(f)

    master_durations = get_all_run_durations(ratios)

    for bc_type in bc_types:
        draw_ratio_vs_lbc(ratios, output_folder, bc_type_sel=bc_type, master_durations=master_durations)

    for lbc in lbcs_all:
        draw_ratio_vs_bc_type(ratios, output_folder, lbc_sel=lbc, master_durations=master_durations)

    for bc_type in bc_types:
        for lbc in lbcs_all:
            draw_ratio_vs_label(ratios, output_folder, bc_type_sel=bc_type, lbc_sel=lbc, master_durations=master_durations)
